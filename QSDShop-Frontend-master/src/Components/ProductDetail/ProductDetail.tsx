...
import { Product } from "../../Pages/women/Women";
...
const ProductDetail = (props: any) => {
  ...
  const { id } = useParams();
  const productId = parseInt(id ? id : "1");
    ...
    <div className={styles.productMain}>
      ...
      <div className={styles.productInfo}>
        <div className={styles.infoContainer}>
          ...
          <h4 className={styles.brandName}>{product.brand.name}</h4>
          <div className={styles.priceContainer}>
            <h4 className={styles.price}>${product.price}</h4>
          </div>
          <div className={styles.sizeGuide}>
            <h4 className={styles.size}>{t("SelectSize")}</h4>
          </div>
          <div className={styles.sizeContainer}>
            <div className={styles.availableSizes}>
              {product.sizes.map((size: any) => (
                <span key={product.id}> {size.size}</span>
              ))}
            </div>
          </div>
